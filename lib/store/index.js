...
class ApplicationStore {
    constructor(sdk, initialData, eventEmitter) {
        ...
        this.getters = {
            ...
            STORES_DETAILS: (store) => store.product.stores_details,
            INSTOCK_LOACTIONS: (store) => store.product.instock_locations,
            LOCATION_DETAILS: (store) => store.product.location_details,
            // Cart Getters
            CART: (store) => store.cart,
            CART_ITEMS: (store) => store.cart.cart_items,
            BUY_NOW_CART_ITEMS: (store) => store.cart.buy_now_cart_items,
            CHECKOUT_MODE: (store) => store.cart.cart_items.checkout_mode,
            ...
            SHARE_CART_ITEMS: (store) => store.cart.share_cart_items,
            // Address Getters
            ...
            CUSTOM_OBJECT: (store) => store.content.custom_object,
            // Auth Getters
        ...
        this.address = createCacheProxy('address', new AddressModule(dispatch), eventEmitter);
        this.products = createCacheProxy('products', new ProductsModule(dispatch), eventEmitter);
        this.product = createCacheProxy('product', new ProductModule(dispatch), eventEmitter);
        this.cart = createCacheProxy('cart', new CartModule(dispatch), eventEmitter);
        this.brands = createCacheProxy('brands', new BrandsModule(dispatch), eventEmitter);
        this.theme = createCacheProxy('theme', new ThemeModule(dispatch), eventEmitter);
        this.content = createCacheProxy('content', new ContentModule(dispatch), eventEmitter);
        this.configuration = createCacheProxy('configuration', new ConfigurationModule(dispatch), eventEmitter);
        this.logistic = createCacheProxy('logistic', new LogisticModule(dispatch), eventEmitter);
        this.payment = createCacheProxy('payment', new PaymentModule(dispatch), eventEmitter);
        this.auth = createCacheProxy('auth', new AuthModule(dispatch), eventEmitter);
        this.share = createCacheProxy('share', new ShareModule(dispatch), eventEmitter);
        this.collection = createCacheProxy('collection', new CollectionModule(dispatch), eventEmitter);
        this.categories = createCacheProxy('categories', new CategoriesModule(dispatch), eventEmitter);
        this.order = createCacheProxy('order', new OrderModule(dispatch), eventEmitter);
        this.custom = createCacheProxy('custom', new CustomModule(dispatch), eventEmitter);
        this.lead = createCacheProxy('lead', new LeadModule(dispatch), eventEmitter);
