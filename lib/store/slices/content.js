...
export const getCustomFields = createAsyncThunk("getCustomFields", (params, { extra }) => __awaiter(void 0, void 0, void 0, function* () {
    const { sdk } = extra;
    return sdk.content.getCustomFields(params);
}));
...
const contentSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            state.custom_field.loading = true;
        })
            .addCase(getCustomFields.fulfilled, (state, action) => {
            state.custom_field = action.payload;
            state.custom_field.error = null;
            state.custom_field.loading = false;
        })
            .addCase(getCustomFields.rejected, (state, action) => {
            state.custom_field = {};
            state.custom_field.error = action.error;
            state.custom_field.loading = false;
        })
            //blog item
